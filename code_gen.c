...
char *opcode_name[] = {"OP_NULL", "LOD", "LDX", "LDXB", "LDA", "LITI", "STO", "STOB", "STX", "STXB", "SUBI", "SUBF", "DIVI", "DIVF", "ADDI", "ADDF", "OFFSET", "MULI", "LEQI", "LEQF", "GEQI", "GEQF", "NEQI", "NEQF", "EQLI", "EQLF", "NOT", "OR", "AND", "CVTI", "CVTF", "JPC", "JPCR", "JMP", "JPT", "JPTR", "INT", "INCI", "INCCF", "DECI", "DECF", "SUP", "CAL", "ADDR", "RET", "MINUSI", "MINUSF", "CHK", "LDI", "LDIB", "SWITCH", "SWVALUE", "SWDEFAULT", "SWLABEL", "SWEND", "POP", "POPB"};
...
void gen_expression_left(A_NODE *node)
{
    ...
    case N_EXP_ARRAY:
        gen_expression(node->llink);
        gen_expression(node->rlink);
        // gen_code_¡(CHK,0,node-> llink-›type-› expr);
        if (node->type->size > 1)
        {
            gen_code_i(LITI, 0, node->type->size);
            gen_code_i(MULI, 0, 0);
...
void gen_error(int i, int ll, char *s)
{
    ...
    case 11:
        ...
        break;
    case 12:
        printf("illegal I-value expression \n'");
        break;
    case 13:
        printf("identifier %s not I-value expression \n", s);
        break;
    case 20:
        printf("illegal default label in switch statement \n");
        break;
    case 21:
        printf("illegal case label in switch statement In");
        break;
    case 22:
        printf("no destination for continue statement \n");
        break;
    case 23:
        printf("no destination for break statement In");
        break;
    case 100:
