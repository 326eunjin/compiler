...
%token IDENTIFIER TYPE_IDENTIFIER AUTO_SYM BREAK_SYM CASE_SYM CONTINUE_SYM DEFAULT_SYM 
	DO_SYM ELSE_SYM ENUM_SYM FOR_SYM IF_SYM RETURN_SYM SIZEOF_SYM STATIC_SYM STRUCT_SYM 
	SWITCH_SYM TYPEDEF_SYM UNION_SYM WHILE_SYM PLUSPLUS MINUSMINUS ARROW LSS GTR LEQ GEQ 
	EQL NEQ AMPAMP BARBAR DOTDOTDOT LP RP LB RB LR RR COLON PERIOD COMMA EXCL STAR SLASH 
	PERCENT AMP SEMICOLON PLUS MINUS ASSIGN INTEGER_CONSTANT FLOAT_CONSTANT STRING_LITERAL 
	CHARACTER_CONSTANT

%start program
%%
program : translation_unit {checkForwardReference();root=makeNode(N_PROGRAM,NIL,$1,NIL);}
translation_unit
...
struct_type_specifier
	: struct_or_union IDENTIFIER {$$=setTypeStructOrEnumIdentifier($1,$2,ID_STRUCT);}LR {$$=current_id; current_level++;} struct_declaration_list RR {$$ = setTypeField($3,$6);checkForwardReference(); current_level--; current_id=$5;}
	| struct_or_union {$$=makeType($1);} LR {$$=current_id; current_level++;} struct_declaration_list RR {$$=setTypeField($2,$5); checkForwardReference(); current_level--;current_id=$4;}
...
labeled_statement
	...
	;
compound_statement
	: LR {$$=current_id; current_level++;} declaration_list_opt statement_list_opt RR {checkForwardReference(); current_level--;current_id=$2;$$=makeNode(N_STMT_COMPOUND,$3,NIL,$4); }
	;
