...
A_TYPE *sem_expression(A_NODE *node) {
    ...
    case N_EXP_SIZE_EXP:
        ...
        if ((node->clink->clink->name != N_EXP_IDENT ||
        ...
        if (id == 0)
            ...
            if (id->type) {
                ...
                if (isAllowableCastingConversion(id->type, node->llink->type))
                    node->llink =
                        convertCastingConversion(node->llink, id->type);
                else
...
int sem_A_TYPE(A_TYPE *t) {
    ...
    case T_FUNC:
        ...
        if (t->expr) {
            ...
            i = i +
                sem_statement(t->expr, i, t->element_type, FALSE, FALSE, FALSE);
        }
        t->local_var_size = i;
        break;
    case T_POINTER:
...
int sem_declaration(A_ID *id, int addr) {
    ...
    case ID_PARM:
        if (id->type) {
            ...
            // usual unary conversion of param type
...
A_LITERAL getTypeAndValueOfExpression(A_NODE *node) {
    ...
    case N_EXP_CAST:
        ...
        result = checkTypeAndConvertLiteral(result, (A_TYPE *)node->llink,
                                            node->line);
        break;
    case N_EXP_MUL:
